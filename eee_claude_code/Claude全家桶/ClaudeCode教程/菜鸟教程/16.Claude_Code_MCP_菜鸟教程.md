...
claude mcp add \--transport stdio \--env AIRTABLE\_API\_KEY\=你的密钥 airtable \\
...
/mcp\_\_jira\_\_create\_issue "Login bug" high
```
...
1.  **身份验证**：远程MCP服务器（如GitHub/Sentry）需在Claude Code中执行 
    ```
    /mcp
    ```
     完成OAuth 2.0授权；
2.  **Windows兼容**：本地stdio服务器若用npx，需加
    ```
    cmd /c
    ```
    包装（如
