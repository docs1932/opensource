...
**控制成本**：将简单任务交给 Haiku，将复杂分析交给 Sonnet，通过环境变量 
```
...
然后选择 **Project (.claude/agents/)**，代理文件会保存到当前目录的 .claude/agents/ 目录下，如果选择 
```
~/.claude/agents/
```
 目录，则对所有项目生效：
...
name
```
 | 必填 | 唯一标识，也是显式调用时的名称。格式：小写字母 + 连字符，如 
```
code-reviewer
```
 |
...
> bypassPermissions
> ```
>  只适合完全可信的子代理。另外，子代理会**继承父会话的权限模式**——如果主会话开启了 bypass，所有子代理也会跟着 bypass，使用时请格外谨慎。
...
```
export CLAUDE\_CODE\_DISABLE\_BACKGROUND\_TASKS\=1
```

如需统一设置所有子代理使用的模型（例如主对话用 Opus 做复杂推理，子代理用 Sonnet 节省成本）：

```
export CLAUDE\_CODE\_SUBAGENT\_MODEL\="claude-sonnet-4-6"
```
---
...
-   **architect-review**：验证设计约束，产出架构决策记录（ADR），标记 
    ```
    READY_FOR_BUILD
    ```
    
-   **implementer-tester**：实现代码和测试，更新文档，标记 
    ```
    DONE
    ```
