...
ANTHROPIC_API_KEY
```
 | Claude API 密钥（从 claude.ai 获取） | API 密钥字符串 |
 
```
ANTHROPIC_BASE_URL
```
 | API 请求的目标地址（用于代理或自定义端点） | URL 地址 |
 
```
ANTHROPIC_AUTH_TOKEN
```
 | 认证令牌（用于 VS Code 插件等场景） | 令牌字符串 |
...
export ANTHROPIC\_BASE\_URL\="https://api.anthropic.com"  
...
    "ANTHROPIC\_BASE\_URL": "https://custom-proxy.com/v1"  
  }  
}  
...
export ANTHROPIC\_MODEL\="claude-opus-4-5"  
export CLAUDE\_CODE\_SUBAGENT\_MODEL\="claude-sonnet-4-5"  
...
```
 | 允许 Claude 使用的工具白名单（逗号分隔） | 工具列表 |
 
```
CLAUDE_CODE_DISALLOWED_TOOLS
```
 | 禁止 Claude 使用的工具黑名单 | 工具列表 |
...
\# 在 CI 环境中使用只读模式  
export CLAUDE\_CODE\_PERMISSION\_MODE\="plan"  
export CLAUDE\_CODE\_ALLOWED\_TOOLS\="Read,Grep,Glob,Bash(gh \*)"  
...
```
 | Vertex AI 项目 ID（使用 Vertex 时由认证步骤设置） | GCP 项目 ID |
...
    "CLAUDE\_CODE\_SUBAGENT\_MODEL": "claude-haiku-3-5",  
    "CLAUDE\_CODE\_PERMISSION\_MODE": "plan",  
    "CLAUDE\_CODE\_ALLOWED\_TOOLS": "Read,Grep,Glob,Bash",  
...
    {  
      "name": "ANTHROPIC\_BASE\_URL",  
      "value": "https://custom-proxy.com/api"  
    },  
    {  
      "name": "ANTHROPIC\_AUTH\_TOKEN",  
      "value": "your-token-here"  
    },  
    {  
      "name": "ANTHROPIC\_MODEL",  
...
export ANTHROPIC\_BASE\_URL\="https://proxy.company.com/anthropic/v1"  
...
export ANTHROPIC\_MODEL\="claude-sonnet-4-5"  
export CLAUDE\_CODE\_SUBAGENT\_MODEL\="claude-haiku-3-5"  
...
\# CI 环境中使用只读模式  
export CLAUDE\_CODE\_PERMISSION\_MODE\="plan"  
export CLAUDE\_CODE\_DISABLE\_HISTORY\="1"  
export CLAUDE\_CODE\_ALLOWED\_TOOLS\="Read,Grep,Glob,Bash(gh \*)"  
...
$env:CLAUDE\_CODE\_USE\_POWERSHELL\_TOOL = "1"  
