...
https://www.jb51.net/server/30719072x.htm (linux 命令重新整理)
...
这时候我们的第一生成证书已经完成了，接下来就是配置我们的web服务器，启用HTTPS。
    server {
        listen       443 ssl;
        server_name  www.cloudloong.cn; ## 重要！！！修改成你的 域名/外网IP【nginx所在机器的ip】

        ssl_certificate      /etc/letsencrypt/live/cloudloong.cn/fullchain.pem;  # ssl证书文件位置
        ssl_certificate_key  /etc/letsencrypt/live/cloudloong.cn/privkey.pem;  # ssl证书key的位置
