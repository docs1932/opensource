...
claude mcp
```
 | 配置 MCP 服务器 | 详见 [Claude Code MCP 文档](https://www.runoob.com/claude-code/claude-code-mcp.html) |
...
3.  **
    ```
    21.-append-system-prompt
    ```
    23.*：保留默认功能，追加定制要求（推荐大多数场景使用）
    ```
...
-p
```
）的详细用法（输出格式、流式传输、程序化集成等），参考 [SDK 文档](https://docs.claude.com/en/docs/agent-sdk)。