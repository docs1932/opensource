...
use_bedrock
```
 | 使用 AWS Bedrock 而非 Claude API | 否 |
...
jobs:
  claude\-pr:
    if: |
      ...
      (github.event\_name \== 'pull\_request\_review\_comment' && contains(github.event.comment.body, '@claude'))
    runs\-on: ubuntu\-latest
    env:
    ...
    steps:
      ...
      \- name: Generate GitHub App token
        ...
        with:
          ...
          private\-key: ${{ secrets.APP\_PRIVATE\_KEY }}
      ...
      \- uses: anthropics/claude\-code\-action@v1
        with:
          github\_token: ${{ steps.app\-token.outputs.token }}
          use\_bedrock: "true"
          claude\_args: '--model us.anthropic.claude-sonnet-4-6 --max-turns 10'
```
---
...
jobs:
  claude\-pr:
    if: github.event\_name \== 'issue\_comment' && contains(github.event.comment.body, '@claude')
    runs\-on: ubuntu\-latest
    steps:
      \- name: Checkout repository
        uses: actions/checkout@v4
      ...
      \- name: Authenticate to Google Cloud
        ...
        with:
          workload\_identity\_provider: ${{ secrets.GCP\_WORKLOAD\_IDENTITY\_PROVIDER }}
          service\_account: ${{ secrets.GCP\_SERVICE\_ACCOUNT }}

      \- uses: anthropics/claude\-code\-action@v1
        with:
          github\_token: ${{ steps.app\-token.outputs.token }}
          trigger\_phrase: "@claude"
          use\_vertex: "true"
          claude\_args: '--model claude-sonnet-4-5@20250929 --max-turns 10'
        env:
          ANTHROPIC\_VERTEX\_PROJECT\_ID: ${{ steps.auth.outputs.project\_id }}
...
\- uses: anthropics/claude\-code\-action@beta
  with:
    mode: "tag"
    direct\_prompt: "Review this PR for security issues"
    anthropic\_api\_key: ${{ secrets.ANTHROPIC\_API\_KEY }}
    custom\_instructions: "Follow our coding standards"
    max\_turns: "10"
    model: "claude-sonnet-4-6"
```
...
jobs:
  claude:
    ...
    steps:
      \- uses: anthropics/claude\-code\-action@v1
        with:
          claude\_args: "--max-turns 5 --allowedTools Read,Grep,Glob,Bash,Write,Edit"
```
